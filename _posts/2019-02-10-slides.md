...
> 自然语言意图 → PEGS需求 → 可检查规格 → Agent候选 → 测试/静态分析/求解器/证明器 → 人工Gate → 运行证据

生成器负责提出候选，外部工具负责施加约束，人类负责目标、风险和最终取舍。评价Agent时同时测量生成吞吐量与验证吞吐量；只增加前者会形成越来越大的未验证变更积压。
...
先用PEGS避免从一句用户故事直接跳到功能：

| Book | 要回答的问题 |
|---|---|
| Project | 谁参与，进度、资源、角色和过程约束是什么？ |
| Environment | 领域事实、现行流程、法律、外部系统与参与者是什么？ |
| Goals | 为什么建设，预期结果和成功指标是什么？ |
| System | 系统承担什么行为、接口及功能/质量约束？ |
...
LLM形式化遵循“小范围、可反馈”的路线：先选择一条关键需求，生成候选验收示例、断言、不变量或时序性质；由类型检查器、测试框架、SMT求解器或证明器返回结果；再由Agent修订并由人审查。不得把可解析或可证明误写成“规格符合真实目标”。
...
形式化方法提供另一类证据：契约验证、模型检查和定理证明能够在明确模型与假设下覆盖一类状态或输入，而不只是执行有限样本。CompCert、seL4等已部署系统说明这条路线可行，但成本、工具、专业能力和验证边界必须显式管理。任何“已验证”声明都必须补全为：哪个版本，在什么假设下，满足哪一份规格的哪些性质。
...
验证器拒绝一个候选时，应把诊断和最小反例反馈给Agent，但需设置停止条件：若连续建议没有形成新的可检验假设，就停止“看似合理的修改 → 验证失败 → 再次生成”的幻觉循环，由人重新检查规格和故障模型。Agent是候选生成器，不是最终裁判。
...
实验：从REQ-017建立追踪链；设计决策表；实现单元、集成和端到端测试；运行覆盖率与变异测试；再选择一个有契约或不变量的小缺陷，比较“无Agent”和“Agent+验证器反馈”的修复过程，记录正确修复、用时、提示次数、被拒候选和停止原因；让外组依据文档复现；形成质量报告。这个小样本课堂实验只支持本班条件下的反思，不支持“AI普遍提高/降低生产率”的宏大结论。
