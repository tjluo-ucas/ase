...
| **3** | **Requirements II: PEGS, specifications, acceptance.** Separate Project, Environment, Goals, and System; turn vague adjectives into scenarios, assertions, invariants, or decidable properties. | Critique ambiguity and premature design; draft a PEGS summary and three key scenarios. | Review, counterexamples, frozen examples, and one agent-generated checkable candidate revised through tool feedback and human judgment. | **A1 (3%)** and baseline. **P1: every core requirement has source, owner, verification method, and specification boundary.** |
...
| **10** | **Verification, formal methods, automated testing, and CI.** Compare sampled tests, properties, contracts, model checking, and proof; measure oracle strength and verification throughput. | Enumerate risk cases; locate a high-coverage weak test; prepare a contract or invariant for a small defect; read a deployed-verification case. | Mutation/fault injection; small verified-fix comparison with and without agent support; detect hallucination loops and execute stopping rules; build CI. | Strategy, frozen set, specification/contract, rejected-candidate trail, defect/coverage report, and CI. **P5: independent oracle and visible unverified boundary.** |
